...
class RemindersListView(APIView):
    """
    API View for getting reminders (leads with follow-up dates).
    
    GET: Returns leads categorized into 4 groups:
        - overdue: Pending reminders with follow_up_at < today (past due)
        - due_today: Pending reminders due today
        - upcoming: Pending reminders with follow_up_at >= tomorrow (future)
        - done: Reminders with follow_up_status='done'
    ...
    def get(self, request, *args, **kwargs):
        """
        Get reminders categorized into 4 groups: overdue, due_today, upcoming, and done.
        """
        # Validate user has profile
        if not hasattr(request.user, 'profile') or request.user.profile is None:
            return Response(
        ...
        # Base queryset: only leads with follow_up_at set, exclude projects
        base_queryset = (
            ...
            .filter(
                is_project=False,
                follow_up_at__isnull=False  # Only leads with follow-up dates
            )
        )
        ...
        if user_role == UserRole.EMPLOYEE.value:
            ...
            assigned_leads = base_queryset.filter(assigned_to=user_profile)
        else:
            # Managers and DEV_LEAD can see all reminders (only assigned leads, not unassigned)
            assigned_leads = base_queryset.filter(assigned_to__isnull=False)
        
        # Category 1: Overdue reminders (pending, follow_up_at < today)
        overdue_reminders = assigned_leads.filter(
            follow_up_at__lt=today_start,
            follow_up_status='pending'
        ).order_by('follow_up_at')
        
        # Category 2: Due today reminders (pending, today's date)
        ...
        # Category 3: Upcoming reminders (pending, tomorrow and after)
        upcoming_reminders = assigned_leads.filter(
            follow_up_at__gte=tomorrow_start,
            follow_up_status='pending'
        ).order_by('follow_up_at')
        
        # Category 4: Done reminders (status is done)
        done_reminders = assigned_leads.filter(
            follow_up_status='done'
        ).order_by('-follow_up_at')
        
        # Serialize each category
        overdue_serializer = LeadSerializer(overdue_reminders, many=True)
        due_today_serializer = LeadSerializer(due_today_reminders, many=True)
        upcoming_serializer = LeadSerializer(upcoming_reminders, many=True)
        done_serializer = LeadSerializer(done_reminders, many=True)
        
        return Response({
            "success": True,
            "overdue": {
                "count": overdue_reminders.count(),
                "reminders": overdue_serializer.data
            },
            "due_today": {
                "count": due_today_reminders.count(),
                "reminders": due_today_serializer.data
            },
            "upcoming": {
                "count": upcoming_reminders.count(),
                "reminders": upcoming_serializer.data
            },
            "done": {
                "count": done_reminders.count(),
                "reminders": done_serializer.data
            },
            "total": {
                "count": overdue_reminders.count() + due_today_reminders.count() + 
                         upcoming_reminders.count() + done_reminders.count()
            }
        }, status=status.HTTP_200_OK)