...
class ManageLeadOptionsView(View):
    """View for managers to manage lead status and source options"""
    template_name = 'ui/manage_lead_options.html'
    ...
    def post(self, request):
        ...
        if action == 'add_status':
            status_name = request.POST.get('status_name', '').strip()
            if status_name:
                LeadStatus.objects.get_or_create(name=status_name)
                messages.success(request, f'Status "{status_name}" added successfully!')
            else:
            ...
            try:
                status_obj = LeadStatus.objects.get(id=status_id)
                status_name = status_obj.name
                
                # Check if status is in use by any leads
                from leads.models import Lead
                leads_using_status = Lead.objects.filter(status=status_name).count()
                if leads_using_status > 0:
                    messages.error(request, f'Cannot delete status "{status_name}" - it is currently used by {leads_using_status} lead(s). Please reassign those leads first.')
                else:
                    status_obj.delete()
                    messages.success(request, f'Status "{status_name}" deleted successfully!')
            ...
            try:
                source_obj = LeadSource.objects.get(id=source_id)
                source_name = source_obj.name
                
                # Check if source is in use by any leads
                from leads.models import Lead
                leads_using_source = Lead.objects.filter(source=source_name).count()
                if leads_using_source > 0:
                    messages.error(request, f'Cannot delete source "{source_name}" - it is currently used by {leads_using_source} lead(s). Please reassign those leads first.')
                else:
                    source_obj.delete()
        ...
        return redirect('/manage-lead-options/')
